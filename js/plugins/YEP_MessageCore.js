...
Window_NameBox.prototype.refresh = function(text, position) {
    ...
    // ADD_名前で色変え（プラグインパラメタは無視）
    var syuzinkou = "神代 イツキ";    
    var saoyaku = ["後藤田 ゲンジ",
                   "冴木 タクマ",
                   "田中 ノブオ",
                   "マッサージ師",
    ...
    var tips = "Tips";
    if (syuzinkou === text) {
        Yanfly.Param.MSGNameBoxText = String("\\C[13]");
    } else if (text.indexOf(tips) != -1) {
        Yanfly.Param.MSGNameBoxText = String("\\C[29]");
    } else if (saoyaku.indexOf(text) >= 0) {
        Yanfly.Param.MSGNameBoxText = String("\\C[20]");
    } else {
        Yanfly.Param.MSGNameBoxText = String("");
    }    
